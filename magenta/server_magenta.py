...
def load_model_and_tokenizer(model_id: str) -> Tuple[transformers.PreTrainedModel, transformers.PreTrainedTokenizer, str]:
    ...
    Args:
        model_id: Identificatore del modello su HuggingFace (es. "google/gemma-3-4b-it")
    ...
    # Aggiunge pad_token se mancante
    if tokenizer.pad_token is None:
        tokenizer.add_special_tokens({"pad_token": tokenizer.eos_token})

    print(f"🤖 Caricamento modello: {model_id}")

    # Argomenti comuni per from_pretrained
    common_model_args = {
        "trust_remote_code": True,
    ...
    if device == "cuda":
        model = transformers.AutoModelForCausalLM.from_pretrained(
            model_id,
            device_map="auto",          # lascia che Accelerate distribuisca
            torch_dtype=torch.bfloat16,  # bfloat16 -> meno VRAM
            **common_model_args,
        )
        # NON chiamare .to() altrimenti errore «You shouldn't move…»
    elif device == "mps":
        model = transformers.AutoModelForCausalLM.from_pretrained(
            model_id,
            torch_dtype=torch.float32,  # MPS non supporta bfloat16 in modo affidabile
            **common_model_args,
        )
        model.to("mps")
    else:  # device == "cpu"
        model = transformers.AutoModelForCausalLM.from_pretrained(
            model_id,
            torch_dtype=torch.float32, # float32 per CPU
            **common_model_args,
        )
        model.to("cpu")
