...
def load_model_and_tokenizer(model_id: str) -> Tuple[transformers.PreTrainedModel, transformers.PreTrainedTokenizer, str]:
    ...
    model_kwargs = {"trust_remote_code": True, "low_cpu_mem_usage": True, "ignore_mismatched_sizes": True}
    
    if device == "cuda":
        model_kwargs["device_map"] = "auto"
        model_kwargs["torch_dtype"] = torch.bfloat16
        # model_kwargs["attn_implementation"] = "sdpa" # Consider for Gemma/newer models
        model = transformers.AutoModelForCausalLM.from_pretrained(model_id, **model_kwargs)
        # Non chiamare .to(device) con device_map="auto"
    elif device == "mps":
        model_kwargs["torch_dtype"] = torch.float32 # MPS non supporta bfloat16 bene
        model = transformers.AutoModelForCausalLM.from_pretrained(model_id, **model_kwargs)
        model.to("mps")
    else:  # CPU
