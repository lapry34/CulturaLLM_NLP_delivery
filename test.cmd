@echo off
REM Windows CMD script for testing API endpoints
...
REM Esempio per il task yellow
...
REM Esempio per il task red
echo [RED TASK] Sending curl request...
echo [Input] 'question': 'Cosa e la bistecca?', 'answer': 'Cibo', 'feedback': 'no'
curl -X POST "http://localhost:8071/red" -H "accept: application/json" -H "Content-Type: application/json" -d "{\"question\": \"Cosa e la bistecca?\",\"answer\": \"Cibo\",\"feedback\": \"no\"}"
echo.
echo [RED TASK] Response received.
echo.

REM Esempio per il task magenta
echo [MAGENTA TASK] Sending curl request...
echo [Input] 'llm_response': 'In qualita di modello linguistico avanzato...', 'level': '1'
curl -X POST "http://localhost:8071/magenta" -H "accept: application/json" -H "Content-Type: application/json" -d "{\"llm_response\": \"In qualita di modello linguistico avanzato, desidero informarla che la sua richiesta e stata elaborata con successo e i risultati sono pronti per la sua disamina\", \"level\": \"1\"}"
echo.
echo [MAGENTA TASK] Response received.
echo.

REM Esempio per il task orange
echo [ORANGE TASK] Sending curl request...
echo [Input] 'question': 'Che cos e il Barocco?'
curl -X POST "http://localhost:8071/orange" -H "accept: application/json" -H "Content-Type: application/json" -d "{\"question\": \"Che cos e il Barocco?\"}"
echo.
echo [ORANGE TASK] Response received.
echo.

REM Esempio per il task cyan
...
REM Esempio per il task green_coherence_QA
echo [GREEN QA TASK] Sending curl request...
echo [Input] 'question': 'Qual e la capitale d Italia?', 'answer': 'La capitale d Italia e Roma.'
curl -X POST "http://localhost:8071/green_coherence_QA" -H "accept: application/json" -H "Content-Type: application/json" -d "{\"question\": \"Qual e la capitale d Italia?\", \"answer\": \"La capitale d Italia e Roma.\"}"
echo.
echo [GREEN QA TASK] Response received.
echo.

REM Esempio per il task green_coherence_QT
...
REM Esempio per il task green cultural
...
REM Esempio per il task green validity
