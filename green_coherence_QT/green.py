#!/usr/bin/env python3
"""
Gemma Few-Shot Agent - Valutatore di Validità delle Risposte
=================================================
Sistema di valutazione automatica della validità delle risposte basato su modelli
...
[
  {
    ...
    "feedback": "Feedback di valutazione",
    "score": <Punteggio da 0 a 5>
  }
]

Author: Alberto Guida
"""
...
- Una breve spiegazione della valutazione (2-3 frasi)
...
def interactive_session(
    ...
    print("\n" + "="*60)
    print("SESSIONE INTERATTIVA - Valutatore di Validità delle Risposte")
    print("="*60)
    ...
    while True:
        ...
        if user_input.lower() == "help":
            print("\n📖 AIUTO:")
            print("- Formato: domanda|risposta")
            print("- Il sistema valuta la validità della risposta (1-5)")
            print("- 1 = non valida, 5 = valida\n")
            continue
        ...
        try:
            ...
            # STEP 6: Estrai e mostra punteggio e feedback
            import re
            try:
                print(result)  # Mostra output grezzo per debug
                # Check if result contains the pipe separator for SCORE|FEEDBACK format
                if '|' in result:
                    parts = result.split('|', 1)  # Split only on first pipe
                    if len(parts) == 2:
                        ...
                        if numbers:
                            score = int(numbers[0])
                            if 1 <= score <= 5:
                                print(f"📊 Punteggio validità: {score}/5")
                                print(f"💬 Feedback: {feedback_part}")
                            else:
                                print(f"⚠️  Valore fuori range (1-5). Score: {score}")
                                print(f"💬 Feedback: {feedback_part}")
                        else:
                            print(f"⚠️  Nessun numero trovato nella parte score. Output grezzo: {result}")
                    else:
                        print(f"⚠️  Formato non valido (manca il separatore |). Output grezzo: {result}")
                else:
                    ...
                    if 1 <= score <= 5:
                        ...
                        print(f"⚠️  Valore fuori range (1-5). Output grezzo: {result}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Gemma Few-Shot Agent - Valutatore di Validità delle Risposte",
        ...
        print("""[
  {
    ...
    try:
        ...
        required_fields = ["question", "answer", "feedback", "score"]
        valid_examples = []
    ...
    try:
        llm = create_safe_pipeline(
            ...
            tokenizer, 
            device,
            max_tokens=50,  # Solo 5 token per un numero
            debug=args.debug
        )
    except Exception as e:
    ...
    chain = build_few_shot_chain(llm, examples, args.debug)
    
    # STEP 8: Avvia sessione interattiva
    interactive_session(chain, args.debug)
